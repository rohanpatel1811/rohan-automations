...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Vercel functions have a 60s wall-clock limit; abort at 55s to leave time for response
  ...
  if (!psiRes.ok) {
    // Surface the PSI error message in-voice
  ...
  // Skip meta/debug audit types; only surface ones with a human-readable displayValue
  const SKIP_TYPES = new Set([
    'debugdata', 'screenshot', 'filmstrip', 'treemap-data',
    'criticalrequestchain', 'table', // table without displayValue is noise
  ])
